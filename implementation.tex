...
\item \textit{Repository} -- encapsulates all interactions with the database. All repositories are based on Spring Data\cite{springdata}.
\item \textit{Score} -- processes the model and calculates the scores. The scores are documented in Section \ref{sec:scores}.
\item \textit{Model} -- contains all nanoentities and coupling data. %TODO document?
\end{itemize}
