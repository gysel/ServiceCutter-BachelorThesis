...
Importing the user representation should either be done over a RESTful HTTP API in a self defined JSON format or using a wizard in the Service Cutter's \gls{UI}.
...
\section{Components and Packages}

The Service Cutter is divided into two parts:
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[scale=1]{diagrams/PackageDiagram.pdf}
		\caption{Components and packages of the Service Cutter}
		\label{fig:packages}
	\end{center}
\end{figure}

The \textit{Editor} consists of four components:

\begin{itemize}
\item \textit{Scripts} -- contains all JavaScript code and \gls{HTML} markup.
\item \textit{Styles} -- includes the required \gls{CSS} files.
\item \textit{Java} -- consists of the Java classes that offer access to the engine to the \gls{UI}.
\item \textit{JHipster} -- includes a set of components provided by JHipster such as user management, authorization, authentication, logging, monitoring and others.
\end{itemize}

The Engine offers a RESTful HTTP web service and is structured into the following five packages:

\begin{itemize}
\item \textit{Importer} -- contains the service endpoint that accepts nanoentities and user representations. %TODO add reference?
\item \textit{Solver} -- accesses the scores to create the graph, runs the algorithm and exposes the candidate service cuts on a web service. The algorithms are documented in Section \ref{sec:algorithms}.
\item \textit{Score} -- processes the model and calculates the scores. The scores are documented in Section \ref{sec:scores}.
\item \textit{Model} -- contains the coupling criteria catalog, all nanoentities and coupling data. The sub package repository encapsulates all interactions with the database. All repositories are based on Spring Data\cite{springdata}. %TODO document?
\end{itemize}
