...
Figure \ref{fig:packages} introduces the most important components and packages of the Service Cutter.
...
\begin{itemize}
\item \textit{Scripts} -- contains all JavaScript code and HTML markup.
\item \textit{Styles} -- includes the requires CSS files.
...
\begin{itemize}
\item \textit{Importer} -- contains the service endpoint that accepts the nanoentities and user representations. %TODO add reference?
...
\item \textit{Score} -- processes the model and calculate the scores. The scores are documented in Section \ref{sec:scores}.
