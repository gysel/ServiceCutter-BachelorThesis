...
\begin{table}[H]
	...
	\begin{tabular}{|p{170pt}|p{200pt}|}
		...
		\hline
		Responsibility \newline Predefined Service Constraint  & ExclusiveGroupCriteriaScorer \\ 
		\hline
...
A ExclusiveGroupCriteriaScorer is used for the criteria \textit{Responsibility} and \textit{Predefined Service Constraint}. The scorer implements the same logic as a CohesiveGroupCriteriaScorer, but additionally sets penalties from a nanoentity inside a group to all other nanoentities which do not share the same group.

The penalty is configurable and differs for different criteria. A \textit{Predefined Service} is a strong constraint and therefore sets a penalty of $-10$. Mixing nanoentities of different responsibility areas in one service is a disadvantage but sometimes justified. A lower penalty of $-5$ is used for the \textit{Responsibility} criterion.
